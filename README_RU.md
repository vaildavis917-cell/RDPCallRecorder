...
2. Запустите установщик от имени Администратора на RDP-сервере
3. На рабочем столе появится ярлык — кликните для открытия настроек
4. Выберите папку для записей и нажмите "Сохранить"
5. Готово! Агент будет автоматически запускаться при каждом входе в RDP
...
|----------|----------|--------------|
| Папка записей | Куда сохранять записи | `D:\CallRecordings` |
...
Файл `config.ini` находится рядом с исполняемым файлом:
